...
|---|---|---|---|
| `05-unet-line-emission/` | `baseline_…` `v5_…` `v6_…` `v7_…` `v9_…` `v12_…` `v15_…` `v16_…` `v17_…` `analysis_…` | all | Every completed version. **v12 is the published reference**; v16/v17 retrain the sweep winner at a *different seed* than the sweep used (see [`RUNS.md`](RUNS.md)) — not reproduction failures. Versions never downloaded were recovered from their push commits. |
